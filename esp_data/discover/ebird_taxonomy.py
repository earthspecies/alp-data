...
between Clements scientific names and eBird species codes. Also map GBIF
to Clements.
"""
...
# ---------------------------------------------------------------------------
# GBIFToClementsConverter — resolve GBIF canonical name → Clements sci name
# ---------------------------------------------------------------------------


class GBIFToClementsConverter:
    """
    Utility for converting GBIF canonical names to Clements scientific names.

    Loads the JSON cache produced by cache_gbif_to_clements_conversions.py.
    """
    ...
    def __init__(
        ...
        """
        precomputed_dir = str(precomputed_dir).rstrip("/")
        path = f"{precomputed_dir}/gbif_to_clements_{year}.json"

        fs = filesystem_from_path(path)
        with fs.open(path, "r") as f:
            self.lookupdict: dict[str, str] = json.load(f)

    def __call__(self, gbif_canonical_name: str) -> tuple[dict[str, str], bool]:
        """Convert a GBIF canonical name to a Clements scientific name.

        Returns
        -------
        tuple[dict[str, str], bool]
            ``({"clements_name": name}, True)`` on success, ``({}, False)`` on failure.
        """
        name = self.lookupdict.get(gbif_canonical_name)
        if name is None:
            return {}, False
        return {"clements_name": name}, True


# ---------------------------------------------------------------------------
# GBIFToClements — convert scientific_name column → clements_name column
# ---------------------------------------------------------------------------


class GBIFToClementsConfig(BaseModel):
    """Configuration for GBIFToClements transform."""

    type: Literal["gbif_to_clements"] = "gbif_to_clements"
    feature: str = Field(
        description="Column name containing GBIF canonical names.",
        default="scientific_name",
    )
    year: int = Field(
        description="eBird taxonomy year (2021–2025).",
...
class GBIFToClements:
    """Transform that adds a ``clements_name`` column by resolving GBIF canonical names."""

    def __init__(self, feature: str = "scientific_name", year: int = 2021) -> None:
        self.feature = feature
        self.converter = GBIFToClementsConverter(year=year)

    @classmethod
    def from_config(cls, cfg: GBIFToClementsConfig) -> "GBIFToClements":
        return cls(**cfg.model_dump(exclude={"type"}))
    ...
    def __call__(self, backend: DataBackend) -> tuple[DataBackend, dict[str, Any]]:
        ...
        for name in unique_names:
            ...
            if ok:
                mapping[name] = info["clements_name"]
                resolved += 1
            else:
                logger.debug(f"Failed to resolve Clements name for GBIF canonical name: {name}")
                failed += 1

        if failed > 0:
            logger.warning(
                f"Failed to resolve {failed}/{len(unique_names)} unique GBIF canonical names"
            )

        backend = backend.map_column(self.feature, mapping=mapping, output_column="clements_name")

        return backend, {
            "feature": self.feature,
            "resolved": resolved,
            "failed": failed,
            "columns_added": ["clements_name"],
        }


register_transform(GBIFToClementsConfig, GBIFToClements)
