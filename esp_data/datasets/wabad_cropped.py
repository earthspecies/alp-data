"""Pre-cropped WABAD dataset.

Each row corresponds to a single deterministic crop (1–30 s, seed 42, tile mode)
of a WABAD recording. Audio is stored as 16 kHz mono WAV on GCS; selection
...
@register_dataset
class WABADCropped(Dataset):
    """Pre-cropped WABAD dataset.

    Description
    -----------
    Deterministic 1–30 second crops of WABAD recordings, stored as 16 kHz
    mono WAV files on GCS. Each crop's selection table is pre-filtered and
    time-shifted relative to the crop start. Cropping was performed with
    seed=42, tile mode, min_duration=1.0 s, max_duration=30.0 s — identical
    to the ``WABADCropped`` class in the NatureLM-audio-data-synth package.

    Each entry contains:

    - an audio clip (1–30 s, 16 kHz mono)
    - a Raven-style selection table for the clip window (times relative to the
      crop start; includes ``partial`` boolean column)
    ...
    info = DatasetInfo(
        ...
        owner="christos",
        split_paths={
            "all": "gs://foundation-model-data/synthetic/cropped/wabad/all.csv",
        },
        version="1.0.0",
        description="Pre-cropped WABAD clips (1–30 s, seed 42, tile, 16 kHz mono).",
        sources="WABAD / zenodo.org",
    ...
    @classmethod
    def from_config(cls, dataset_config: DatasetConfig) -> tuple["WABADCropped", dict[str, Any]]:
        ...
        Returns
        -------
        tuple[WABADCropped, dict[str, Any]]
