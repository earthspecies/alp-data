"""Synthetic SED Diarization dataset.

Synthetically generated soundscape recordings with speaker-level diarization
annotations, stored on GCS at 16 kHz. Each scene is a multi-species audio
recording with a paired selection table that includes individual speaker labels.
...
_GCS_BASE = "gs://foundation-model-data/synthetic/synthetic_sed_diarization_16k"


@register_dataset
class SyntheticSEDDiarization(Dataset):
    """Synthetic SED Diarization soundscape recordings with speaker-level annotations.

    Description
    -----------
    Synthetically generated multi-species soundscape recordings at 16 kHz,
    paired with Raven-format selection tables that include individual speaker
    diarization labels. In addition to standard ``Begin Time (s)``,
    ``End Time (s)`` fields, the selection table includes ``Annotation``
    (species), ``Speaker`` (individual label, e.g. ``"Corvus levaillantii_1"``),
    ``SNR (dB)``, ``Source Stem``, and ``Source File`` columns.
    ...
    - a Raven-format selection table with diarization labels (DataFrame)
    - ``fn``, ``audio_duration``

    References
    ----------
    gs://foundation-model-data/synthetic/synthetic_sed_diarization_16k
    """

    info = DatasetInfo(
        name="synthetic_sed_diarization",
        owner="christos",
        split_paths={
            "all": f"{_GCS_BASE}/manifest.parquet",
        },
        version="1.0.0",
        description="Synthetically generated SED diarization soundscapes at 16 kHz with speaker-level annotations.",
        sources="gs://foundation-model-data/synthetic/synthetic_sed_diarization_16k",
        license="CC-BY-4.0",
    )
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if idx < 0 or idx >= len(self):
            raise IndexError(f"index {idx} out of range for SyntheticSEDDiarization with {len(self)} samples")
    ...
    @classmethod
    def from_config(cls, dataset_config: DatasetConfig) -> tuple["SyntheticSEDDiarization", dict[str, Any]]:
        ...
        Returns
        -------
        tuple[SyntheticSEDDiarization, dict[str, Any]]
