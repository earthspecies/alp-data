"""Pre-cropped BirdVox-full-night dataset.

Each row corresponds to a single deterministic crop (1–30 s, seed 42, tile mode)
of a BirdVox-full-night recording. Audio is stored as 16 kHz mono WAV on GCS;
selection tables have already been filtered and time-shifted to the crop window.

BirdVox-full-night consists of 6 full-night recordings (~10 h each, 24 kHz)
with expert annotations of nocturnal bird flight calls.

References
----------
https://zenodo.org/records/1010806
Lostanlen et al., "BirdVox-full-night: a dataset and benchmark for avian
flight call detection", ICASSP 2018.
"""
...
@register_dataset
class BirdVoxFullNightCropped(Dataset):
    """Pre-cropped BirdVox-full-night dataset.

    Description
    -----------
    Deterministic 1–30 second crops of BirdVox-full-night recordings (~10 h
    each), stored as 16 kHz mono WAV files on GCS. Each crop's selection
    table is pre-filtered and time-shifted relative to the crop start.
    Cropping was performed with seed=42, tile mode, min_duration=1.0 s,
    max_duration=30.0 s — identical to the ``BirdVoxFullNightCropped`` class
    in the NatureLM-audio-data-synth package.

    BirdVox-full-night contains expert annotations of nocturnal passerine
    flight calls. Annotation columns: ``Begin Time (s)``, ``End Time (s)``,
    ``Low Freq (Hz)``, ``High Freq (Hz)``.  Both frequency columns equal the
    centre frequency (point annotation; no frequency band).

    Each entry contains:

    - an audio clip (1–30 s, 16 kHz mono)
    - a Raven-style selection table for the clip window (times relative to
      the crop start; includes ``partial`` boolean column)
    ...
    info = DatasetInfo(
        name="birdvox_full_night_cropped",
        owner="christos",
        split_paths={
            "all": "gs://foundation-model-data/synthetic/cropped/birdvox_full_night/all.csv",
        },
        version="1.0.0",
        description="Pre-cropped BirdVox-full-night clips (1–30 s, seed 42, tile, 16 kHz mono).",
        sources="zenodo.org/records/1010806",
        license="CC-BY-4.0",
    )
    ...
    ) -> tuple["BirdVoxFullNightCropped", dict[str, Any]]:
