"""Pre-cropped Birdeep dataset.

Each row corresponds to a single deterministic crop (1–30 s, seed 42, tile mode)
of a Birdeep recording. Audio is stored as 16 kHz mono WAV on GCS; selection
...
@register_dataset
class BirdeepCropped(Dataset):
    """Pre-cropped Birdeep dataset.

    Description
    -----------
    Deterministic 1–30 second crops of Birdeep recordings (~60 s AudioMoth
    clips), stored as 16 kHz mono WAV files on GCS. Each crop's selection
    ...
    max_duration=30.0 s — identical to the ``BirdeepCropped`` class in the
    NatureLM-audio-data-synth package.
    ...
    info = DatasetInfo(
        name="birdeep_cropped",
        owner="christos",
        split_paths={
            "all": "gs://foundation-model-data/synthetic/cropped/birdeep/all.csv",
        },
        version="1.0.0",
        description="Pre-cropped Birdeep clips (1–30 s, seed 42, tile, 16 kHz mono).",
        sources="HuggingFace / GrunCrow/BIRDeep_AudioAnnotations",
        license="MIT",
    )
    ...
    @classmethod
    def from_config(cls, dataset_config: DatasetConfig) -> tuple["BirdeepCropped", dict[str, Any]]:
        ...
        Returns
        -------
        tuple[BirdeepCropped, dict[str, Any]]
