...
class XenoCanto(Dataset):
    ...
    **Additional Fields:**
    ...
    - ``train_strong_unseen_thr02_bgdet``: Unseen-safe pseudo-strong training
      set built with a 0.2 classifier threshold and keeping only clips where at
      least one background species was confirmed in at least one window.
    - ``train_strong_unseen_top100_bgdet``: Unseen-safe pseudo-strong training
      set built from top-100 classifier logits, allowing multiple metadata-listed
      background species to be confirmed in the same window when they have strong
      support, and keeping only clips where at least one background species was
      confirmed in at least one window.
