...
def _parse_messages_json(value: str | list) -> str | list:  # noqa: ANN401
    """Parse JSON-encoded chat messages into structured turns.

    Returns
    -------
    str | list
        Parsed messages list, or original value if not a string.
    """
...
class AudioSkillsXL(Dataset):
    ...
    **AudioSet-backed sources** (audio from AudioSet v0.2.0 on GCS):
    ...
    - ``audioset`` -- Full AudioSet conversation QA (~1.6 M rows with audio).
    ...
    def __init__(
        ...
        """Initialize the AudioSkillsXL dataset.
        ...
        ------
        ValueError
            If ``sources`` contains unknown source names.
        """
    ...
    def available_sources(self) -> list[str]:
        """Return the sources present in the currently-loaded data."""
    ...
    def _resolve_split_path(self, source: str) -> str:
        """Resolve a source name to its CSV path, preferring local files.
    ...
    def _load_source(self, source: str) -> DataBackend:
        """Load a single source CSV into a backend.

        Returns
        -------
        DataBackend
            Backend wrapping the loaded source data.
        """
    ...
    def _normalize_messages_backend(self, backend: DataBackend) -> DataBackend:
        ...
        -------
        DataBackend
            Backend with normalised messages column.
        """
    ...
    def _process(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        -------
        dict[str, Any]
            The processed row with ``audio`` and parsed ``messages``.
        """
    ...
    def from_config(cls, dataset_config: DatasetConfig) -> tuple["AudioSkillsXL", dict[str, Any]]:
        ...
        Returns
        -------
        tuple[AudioSkillsXL, dict[str, Any]]
