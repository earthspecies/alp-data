...
from esp_data.io import AnyPathT, PureGSPath, PureR2Path, PureS3Path, anypath, filesystem_from_path
...
def _is_remote_path(path: Any) -> bool:
    """Return True when *path* is a non-local filesystem path."""
    if not isinstance(path, str | Path):
        return False
    path_obj = anypath(str(path))
    return isinstance(path_obj, PureGSPath | PureR2Path | PureS3Path)


def _open_remote_binary(path: str | Path) -> BinaryIO:
    """Open a remote path in binary mode using the configured filesystem."""
        ...
        **kwargs : Any
        ...
        if streaming:
            ...
            if _is_remote_path(path):
                with _open_remote_binary(path) as f:
                    df = pl.read_csv(f, **filtered_kwargs)
            else:
        ...
        **kwargs : Any
        ...
        if streaming:
            ...
            if lines:
                if _is_remote_path(path):
                    with _open_remote_binary(path) as f:
                        df = pl.read_ndjson(f, **filtered_kwargs)
                else:
                    df = pl.read_ndjson(path, **filtered_kwargs)
            else:
                if _is_remote_path(path):
                    with _open_remote_binary(path) as f:
                        df = pl.read_json(f, **filtered_kwargs)
                else:
