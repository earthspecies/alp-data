#!/usr/bin/env python3
"""Build the ``weldy-call-or-song-6s`` BEANS-Pro split.

Same row set as ``weldy-call-or-song`` (we read the deployed 2-s JSONL from
GCS to keep the eval *paired* per detection), but each segment is the target
window plus the 2 s immediately before and after — a 6 s contiguous slice of
the source recording centred on the 2-s target.

At recording boundaries we pad with silence to keep every segment exactly 6 s.

Each emitted row's ``metadata`` carries a ``flank_annotations`` summary so
label-leakage can be quantified post-eval: counts of *other* annotations
overlapping [begin-2, begin] and [end, end+2], split by category and by
whether they're the same species + same sonotype as the target.

Usage::

    uv run python scripts/build_beans_pro_weldy_call_or_song_6s.py \
        --output-dir data/beans_pro_weldy_call_or_song_6s
"""
...
TARGET_SR = 32_000
FLANK_S = 2.0          # 2 s of real audio on each side
SEGMENT_S = 6.0        # total: 2 (pre) + 2 (target) + 2 (post)
LICENSE_STR = "CC-BY-4.0"
SOURCE_DATASET = "weldy_dawn_chorus"
DATASET_NAME = "weldy-call-or-song-6s"
...
def _cut_6s_with_pad(audio: np.ndarray, begin_s: float, end_s: float) -> np.ndarray:
    """Return a 6-s mono PCM16-ready segment centred on ``[begin_s, end_s]``,
    silence-padding at recording boundaries.

    Parameters
    ----------
    audio : np.ndarray
        Full clip at ``TARGET_SR``, mono float32.
    begin_s, end_s : float
        The original 2-s target window.

    Returns
    -------
    np.ndarray
        Float32 segment of length ``SEGMENT_S * TARGET_SR``, peak-normalised.
    """
    sr = TARGET_SR
    ...
    n = audio.shape[-1]
    # Source-side clamp + leading/trailing silence pad.
    a = max(0, a_t)
    b = min(n, b_t)
    seg = audio[a:b]
    out = np.zeros(target_len, dtype=np.float32)
    # Place seg at the appropriate offset within the 6 s output.
    out_start = max(0, -a_t)  # how many silence samples at the start
    out[out_start:out_start + seg.shape[-1]] = seg
    peak = float(np.max(np.abs(out)) or 1.0)
    return (out / peak * 0.97).astype(np.float32)


# ── Flank-annotation quantification ──────────────────────────────────────
...
def _summarise_flanks(
    ...
    *,
    ...
    """For one source clip's selection_table, summarise OTHER annotations whose
    time intervals overlap the flank regions ``[begin-2, begin]`` and
    ``[end, end+2]``.

    Returns a dict bundled into the row's metadata so a post-eval analysis can
    compute, per-row and aggregate, how often the flanks carried potentially
    leaking annotations of the same species/sonotype.
    """
    ...
    def _summarise(rows: list, side: str) -> dict:
        cnt = {
            "n_total": len(rows),
        ...
        for r in rows:
            ...
            sonotype_raw = str(r.get("Sonotype") or "").strip().lower()
            if cat == "species":
                ...
                if "drum" in sonotype_raw:
                    cnt["n_drum"] += 1
            elif cat == "non-biotic":
                cnt["n_non_biotic"] += 1
            elif cat == "method":
                if sonotype_raw in ("empty", "complete"):
                    cnt["n_method_empty"] += 1
        return cnt

    return {
        "pre_2s": _summarise(pre_rows, "pre"),
        "post_2s": _summarise(post_rows, "post"),
    }
...
def build(*, output_dir: Path) -> None:
    ...
    logger.info("Indexed %d recordings", len(recording_index))

    # Group target rows by recording so we only download each clip once.
    by_clip: dict[str, list[dict]] = defaultdict(list)
    for r in tight_rows:
        meta = json.loads(r["metadata"])
        rec_id = meta["recording_id"]
        by_clip[rec_id].append({
            "tight_row": r,
            "meta": meta,
        })
    logger.info("Source clips to read: %d", len(by_clip))

    fs = fsspec.filesystem("gs")
    n_emitted = 0
    n_pad_edge = 0
    jsonl_rows: list[dict] = []
    leakage_counter: Counter = Counter()

    for rec_id, items in by_clip.items():
        entry = recording_index.get(rec_id)
        if entry is None:
            logger.warning("recording_id not in manifest: %s — skipping %d rows", rec_id, len(items))
            continue
        audio_path, st = entry
        try:
        ...
        for item in items:
            ...
            # Cut + pad to exactly 6 s
            seg = _cut_6s_with_pad(audio, begin, end)
            ...
            for side in ("pre_2s", "post_2s"):
                for key, val in flank[side].items():
                    leakage_counter[(side, key)] += val

            # Filename mirrors the tight version with a ``_6s`` suffix.
            tight_fname = Path(tight["audio_path_original_sample_rate"]).stem
            seg_filename = f"{tight_fname}_6s.wav"
            ...
            n_emitted += 1
        if n_emitted % 500 == 0 or n_emitted == len(tight_rows):
            logger.info("Emitted %d / %d segments", n_emitted, len(tight_rows))
    ...
    logger.info("Wrote %d rows to %s", len(jsonl_rows), jsonl_path)
    logger.info("Edge-padded rows (target near recording boundary): %d", n_pad_edge)

    # Quick aggregate leakage summary at build time (full per-row stats are in metadata).
    logger.info("\n=== Aggregate flank-annotation stats across all rows ===")
    ...
    for r in jsonl_rows:
        ...
        same_sp_same = fl["pre_2s"]["n_same_species_same_sonotype"] + fl["post_2s"]["n_same_species_same_sonotype"]
        other_cs = fl["pre_2s"]["n_other_species_call_or_song"] + fl["post_2s"]["n_other_species_call_or_song"]
        drum = fl["pre_2s"]["n_drum"] + fl["post_2s"]["n_drum"]
        species_cat = fl["pre_2s"]["n_species_category"] + fl["post_2s"]["n_species_category"]
    ...
    logger.info("  rows with same-species same-sonotype in either flank: %d (%.1f%%)",
                 rows_with_same_sp_same_sonotype, 100 * rows_with_same_sp_same_sonotype / n_rows)
    logger.info("  rows with other-species call/song in either flank:   %d (%.1f%%)",
                 rows_with_other_call_or_song, 100 * rows_with_other_call_or_song / n_rows)
    logger.info("  rows with drum annotation in either flank:           %d (%.1f%%)",
                 rows_with_drum_in_flank, 100 * rows_with_drum_in_flank / n_rows)
    logger.info("  rows with NO species annotations in either flank:    %d (%.1f%%)",
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=REPO_ROOT / "data" / "beans_pro_weldy_call_or_song_6s")
