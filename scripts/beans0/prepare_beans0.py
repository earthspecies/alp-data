...
DATASET_JSONL_PATHS = {
    ...
    "unseen-species-cmn": "gs://foundation-model-data/data/unseen-species-species-common-classification/v1.32/test_species_license.jsonl",
    "unseen-species-sci": "gs://foundation-model-data/data/unseen-species-species-sci-classification/v1.32/test_species_license.jsonl",
    "unseen-species-tax": "gs://foundation-model-data/data/unseen-species-taxonomic-classification/v1.32/test_species_license.jsonl",
...
TASKS = {
    ...
    "unseen-species-sci": "classification",
    "unseen-species-tax": "classification",
...
def read_jsonl_local(path: str | AnyPath) -> list[dict]:
    try:
...
class BeansSample(DataSample):
    ...
    label: str = Field(default=None, description="Optional label for the audio")

    # optional
    text: Optional[str] = Field(default=None, description="Some text caption")
    task: Optional[str] = Field(default=None, description="The task for the model")


class BeansSampleNoAudio(DataSample):
    """Defines the structure of a Beans0 sample.
...
def make_sample(
    ...
    Args:
        ...
        no_audio (bool): Whether to include audio in the sample.
        output_dir (str): The directory to save the audio to.

    Returns:
        dict: The BeansSample.
    """
    ...
    if "license" in row:
        license = row["license"]
    if "recordist" in row:
        metadata = {"recordist": row["recordist"]}
    if "url" in row:
...
def make_component_dataset(
    ...
    no_audio: bool = False,
    no_audio_output_dir: str | os.PathLike | AnyPath = None,
):
    ...
    Args:
        ...
        use_local_cache (bool): Whether to use local cache to save batches before uploading to GCS.
        no_audio (bool): Whether or not to include audio in the dataset. If True, only metadata is saved.
        no_audio_output_dir (str): The directory to save the raw audio files to in case no_audio is True.
    """
    ...
    for idx, row in tqdm(enumerate(rows, start=state["idx"]), total=len(rows) - state["idx"]):
        ...
        sample: dict = make_sample(
            ...
            no_audio=no_audio,
            output_dir=no_audio_output_dir,
        )
...
def main():
    ...
    parser.add_argument(
        "--no_audio",
        action="store_true",
        help="Whether to include audio within the dataset arrow files",
    )
    parser.add_argument(
        "--no_audio_output_dir",
        type=str,
        default=None,
        help="The directory to save the dataset with no audio to",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
