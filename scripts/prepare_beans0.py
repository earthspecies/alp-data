...
def load_beans_metadata():
    ...
    metadata["gibbons"] = [b for b in beans_cfg if "gibbons" in b["name"]]
...
def read_jsonl_from_bucket(file_path: str | os.PathLike) -> list[dict]:
    ...
    with fs.open(str(file_path)) as f:
        try:
            ...
            print(f"Error reading jsonl {e}, trying line by line")
            # read lines
            records = f.readlines()
...
def make_sample(row: dict, source_dataset: str, license: str, replace_16k: bool = True) -> dict:
    ...
    if not audio_file.exists:
        print(f"Sample not found at {path}")
        return None
...
def make_component_dataset(
    dataset_cfg, output_dir: str | os.PathLike, batch_size: int = 100, replace_16k: bool = True, state: dict = None
):
    ...
    # read the jsonl
    rows = read_jsonl_from_bucket(dataset_cfg["path"], fs)
    print(f"#### PROCESSING DATASET {name} with {len(rows)} samples ####\n")
    ...
    for idx, row in tqdm(enumerate(rows, start=state["idx"]), total=len(rows) - state["idx"]):
        ...
        # save state, over-write local file
        with open("state.json", "w") as f:
            state["config"] = config.to_dict(make_serializable=True)
            json.dump(state, f)
...
def main():
    ...
    for name in args.components_to_process:
        dataset_cfg = {
            ...
            "license": LICENSES.get(name, "unknown"),
            "version": args.versions[ALL_DATASET_NAMES.index(name)],
