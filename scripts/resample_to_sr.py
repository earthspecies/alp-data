...
# ]
# ///
"""Resample a (possibly nested) GCS audio directory to a target sample rate.

Generalisation of ``resample_to_32khz.py`` with a ``--target-sr`` flag. Lists
every audio object under ``--source-prefix`` (a ``gs://bucket/path/`` URI),
downloads each blob, resamples to ``--target-sr`` with librosa kaiser_best, and
uploads the result as a 16-bit PCM WAV to ``--dest-prefix`` (mirroring the
source layout). Files already at the target rate are re-encoded to WAV without
resampling.

Heavy I/O and listing happen inside the job (intended to run on a Slurm CPU
node), never on the login VM.
"""
...
def list_source_blobs(source_uri: str) -> list[str]:
    ...
    -------
    list[str]
        Object keys under the prefix whose names end in an audio extension.
    """
...
def process_batch(args: tuple[list[str], str, str, str, str, str | None, int]) -> tuple[int, int]:
    src_keys, src_bucket_name, src_prefix, dst_bucket_name, dst_prefix, project, target_sr = args
...
def main() -> None:
    ...
    print(f"Target SR: {target_sr}  Workers: {workers}  batch: {args.batch_size}", flush=True)
    ...
    if args.skip_existing:
        ...
        dropped = before - len(src_keys)
        print(f"  remaining after skip: {len(src_keys)} (dropped {dropped})", flush=True)
